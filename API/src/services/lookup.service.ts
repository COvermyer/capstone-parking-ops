...
class LookupService {
    ...
    async initialize() {
        ...
        try {
            // User Role Codes
            let userRoles = await userRoleCodeService.getUserRoles();
            userRoles.forEach(role => {
                this.userRoles.set(role.role_id, role.role_name);
            });

            // State Codes
            let stateCodes = await stateCodeService.getAllStateCodes();
            stateCodes.forEach(state => {
                this.stateCodes.set(state.state_id, [state.state_name, state.state_abbreviation]);
            });

            // Permit Status Codes
            let permitStatusCodes = await permitStatusCodeService.getPermitStatusCodes();
            permitStatusCodes.forEach(status => {
                this.permitStatusCodes.set(status.status_code, status.status_message);
            });

            // appeal status codes
            let appealStatusCodes = await appealStatusCodeService.getAppealStatusCodes();
            appealStatusCodes.forEach(status => {
                this.appealStatusCodes.set(status.appeal_status_code, status.status_message);
            });

            // color codes
            let colorCodes = await colorCodeService.getColorCodes();
